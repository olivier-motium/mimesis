...
**Managed policy settings:** Enterprise administrators can use `allowManagedHooksOnly` to block user, project, and plugin hooks. See Hook configuration.
...
In addition to settings files and plugins, hooks can be defined directly in Skills, subagents, and slash commands using frontmatter. These hooks are scoped to the component's lifecycle and only run when that component is active.
...
- **once**: Set to true to run the hook only once per session. After the first successful execution, the hook is removed.

Note: This option is currently only supported for skills and slash commands, not for agents.
...
- **Exit code 2**: Blocking error. Only stderr is used as the error message and fed back to Claude. The format is `[command]: {stderr}`. JSON in stdout is not processed for exit code 2.
...
- JSON with additionalContext (structured): Use the JSON format below for more control.

**Blocking prompts:**
- `"decision": "block"` prevents the prompt from being processed. The submitted prompt is erased from context.
...
- **Timeout**: 60-second execution limit by default, configurable per command. A timeout for an individual command does not affect the other commands.
...
- **Inspect hook execution** - Use `claude --debug` to see detailed hook execution
