...
| `agent` | Specify which agent type to use when `context: fork` is set. Only applicable when combined with `context: fork`. | `general-purpose` |
...
The Skill tool allows Claude to programmatically invoke both custom slash commands and Agent Skills during a conversation. This gives Claude the ability to use these capabilities on your behalf when appropriate.
...
> The `user-invocable` field in Skills only controls menu visibility, not Skill tool access. Use `disable-model-invocation: true` to block programmatic invocation. See Control Skill visibility for details.
